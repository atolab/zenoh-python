...
class z_timestamp_t(Structure):
    ...
    def __hash__(self):
        return hash((self.time, self.clock_id[0], self.clock_id[1], self.clock_id[2], self.clock_id[3],
            self.clock_id[4], self.clock_id[5], self.clock_id[6], self.clock_id[7],
            self.clock_id[8], self.clock_id[9], self.clock_id[10], self.clock_id[11],
            self.clock_id[12], self.clock_id[13], self.clock_id[14], self.clock_id[15]))
    ...
    def floattime(self):
        ...
        Warning: the time might be rounded, depending the float precision on your host.
        """
        sec = self.time >> 32
        frac = self.time & 0xffffffff
        ns = float(frac) / 0x100000000    # number of NTP fraction per second (2^32)
        return sec+ns

    def datetime(self, tzinfo):
        """
        Return the Timestamp's creation time as a datetime.datetime
        Warning: the time is rounded to milliseconds as datetime precision is millisecond.
        """
